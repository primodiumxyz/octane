...
exports.signGeneratedTransaction = void 0;
 ...
 */
async function signGeneratedTransaction(connection, transaction, feePayer, messageTokenKey, messageToken, cache) {
    // Check that we actually produced this transaction previously
    if (!core_1.MessageToken.isValid(messageTokenKey, transaction.compileMessage(), messageToken, feePayer.publicKey)) {
    ...
    if (await cache.get(key))
        throw new Error('Duplicate signature request');
    ...
    const hasFeePayerSignaturePlaceholder = (transaction.signatures[0].publicKey.equals(feePayer.publicKey)
    ...
    await (0, core_1.simulateRawTransaction)(connection, serializedTransaction);
    return { signature: bs58_1.default.encode(transaction.signature) };
}
exports.signGeneratedTransaction = signGeneratedTransaction;
