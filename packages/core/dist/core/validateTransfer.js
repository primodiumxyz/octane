...
async function validateTransfer(connection, transaction, allowedTokens) {
    ...
    const instruction = (0, spl_token_1.decodeInstruction)(first);
    if (!((0, spl_token_1.isTransferInstruction)(instruction) || (0, spl_token_1.isTransferCheckedInstruction)(instruction)))
        throw new Error('invalid instruction');
    const { keys: { source, destination, owner }, data: { amount }, } = instruction;
    // Check that the source account exists, has the correct owner, is not frozen, and has enough funds
    const account = await (0, spl_token_1.getAccount)(connection, source.pubkey, 'confirmed');
    ...
    if ((0, spl_token_1.isTransferCheckedInstruction)(instruction)) {
        const { keys: { mint }, data: { decimals }, } = instruction;
